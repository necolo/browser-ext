...
var selectedsearch, searchgoogle, searchbing, searchduckduckgo, searchbaidu, searchyandex, typepanelzone, typepanelcustom, typepanellasttime, websitestartname, websitelasttime, navtop, navbottom, navhidden, opentab, opencopy, opennonebookmarks, openbrowserbookmarks, openquickbookmarks, googlesidepanel, zoom, defaultzoom, step, multipletabs, multivalues, navbuttons, gobutton, typehomezone, typehomecustom, websitehomepagename, preventclose, dragnewtab, mutetab, searchyahoo, search360, searchsogou, searchchatgpt, searchgemini, searchwikipedia, disablehorizontalscroll, openallnewtab, refreshtime, autorefresh, showrefreshpanel, zoomvisible, custombookmarks;
...
function saveBookmarks(){
	chrome.storage.sync.set({"custombookmarks": custombookmarks});
}
...
function addBookmark(url, title){
	...
	var exists = custombookmarks.some(function(bookmark){
...
function removeBookmark(url){
	if(custombookmarks){
		custombookmarks = custombookmarks.filter(function(bookmark){
			return bookmark.url !== url;
		});
		saveBookmarks();
	}
...
function showBookmarkAddedMessage(){
	var div = document.createElement("div");
	div.setAttribute("id", "bookmarkaddedmessage");
	div.className = "stefanvdremote";
	document.body.appendChild(div);

	var h3 = document.createElement("h3");
	h3.innerText = "Bookmark Added";
	div.appendChild(h3);

	var p = document.createElement("p");
	p.innerText = "Website has been bookmarked successfully!";
	div.appendChild(p);

	window.setTimeout(function(){
		var element = document.getElementById("bookmarkaddedmessage");
		if(element){
...
function renderBookmarkList(){
	...
	if(!custombookmarks || custombookmarks.length === 0){
		...
		emptyText.style.padding = "16px";
		emptyText.style.color = "var(--ext-primary-font-color)";
	...
	custombookmarks.forEach(function(bookmark){
		...
		link.addEventListener("click", function(event){
			...
			document.getElementById("menubookmarklist").className = "hidden";
			isMenuClick = false;
		});
...
function init(){
	...
	chrome.storage.sync.get(["firstDate", "optionskipremember", "navtop", "navbottom", "navhidden", "typepanelzone", "typepanelcustom", "typepanellasttime", "websitestartname", "websitelasttime", "searchgoogle", "searchbing", "searchduckduckgo", "searchbaidu", "searchyandex", "opentab", "opencopy", "opennonebookmarks", "openbrowserbookmarks", "openquickbookmarks", "websitename1", "websiteurl1", "websitename2", "websiteurl2", "websitename3", "websiteurl3", "websitename4", "websiteurl4", "websitename5", "websiteurl5", "websitename6", "websiteurl6", "websitename7", "websiteurl7", "websitename8", "websiteurl8", "websitename9", "websiteurl9", "websitename10", "websiteurl10", "googlesidepanel", "zoom", "defaultzoom", "step", "multipletabs", "multivalues", "navbuttons", "gobutton", "typehomezone", "typehomecustom", "websitehomepagename", "preventclose", "dragnewtab", "mutetab", "searchyahoo", "search360", "searchsogou", "searchchatgpt", "searchgemini", "searchwikipedia", "disablehorizontalscroll", "openallnewtab", "refreshtime", "autorefresh", "showrefreshpanel", "zoomvisible", "custombookmarks"], function(items){
		...
		custombookmarks = items["custombookmarks"]; if(custombookmarks == null){ custombookmarks = []; }
		...
		document.getElementById("btnaddbookmark").addEventListener("click", function(){
			...
			if(currentUrl && currentUrl !== emptypage){
				addBookmark(currentUrl, currentTitle);
			}
		});

		// Bookmark list button event listener
		document.getElementById("btnbookmarklist").addEventListener("click", function(){
			if(document.getElementById("menubookmarklist").className == ""){
				document.getElementById("menubookmarklist").className = "hidden";
			}else{
				isMenuClick = true;
				renderBookmarkList();
				document.getElementById("menubookmarklist").className = "";
			}
		});
